...
function inject(script, content, file) {
  log.debug(file, 'changed, compiling...', Date.now() - lastInject[script.scriptId] > config.watchThrottle)
  const src = content.code || content
  if (srcCache[script.scriptId] === src) {
    if (!config.watchThrottle || Date.now() - lastInject[script.scriptId] > config.watchThrottle)
      refresh(file)
  ...
  save(script)
  lastInject[script.scriptId] = Date.now()
